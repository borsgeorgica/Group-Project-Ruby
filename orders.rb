...
def get_pizza_type(pizza)
    if pizza.downcase.include? "milano"
            ...
            type = "Ardente"
    elsif pizza.downcase.include? "sicilia"
            type = "Sicilia"    
    elsif pizza.downcase.include? "verona"
            type = "Verona"
    elsif pizza.downcase.include? "roma"
            type = "Roma" 
    elsif pizza.downcase.include? "cheese and tomato"
            type = "Cheese and Tomato" 
    elsif pizza.downcase.include? "mighty meat"
            type = "Mighty Meat" 
    elsif pizza.downcase.include? "pepperoni passion"
            type = "Pepperoni passion" 
    elsif pizza.downcase.include? "texas bbq"
            type = "Texas BBQ" 
    elsif pizza.downcase.include? "vegi supreme"
            type = "Vegi Supreme" 
    elsif pizza.downcase.include? "tandoori hot"
            type = "Tandoori Hot" 
    elsif pizza.downcase.include? "chicken feast"
            type = "Chicken feast" 
    elsif pizza.downcase.include? "hot And spicy"
            type = "Hot And Spicy" 
    elsif pizza.downcase.include? "vegi volcano"
            type = "Vegi Volcano" 
    elsif pizza.downcase.include? "new yorker"
            type = "New Yorker" 
